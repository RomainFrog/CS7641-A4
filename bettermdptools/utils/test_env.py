...
class TestEnv:
    ...
    def test_env(env, desc=None, render=False, n_iters=10, pi=None, user_input=False, convert_state_obs=lambda state: state):
        ...
        run_length = np.full([n_iters], np.nan)
        endings = []
        for i in range(0, n_iters):
            ...
            while not done:
                ...
                if truncated:
                    endings.append("T")
                if terminated:
                    if reward == 1 - (env.step_penalty if hasattr(env, 'step_penalty') else 0):
                        endings.append("G")
                    else:
                        endings.append("H")
            ...
            run_length[i] = step_count
        env.close()
        return test_scores, run_length, endings
