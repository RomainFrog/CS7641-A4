...
class RL:
    ...
    def q_learning(self,
        ...
        endings = [] # describes how the episode ended ("T": truncated, "G": goal, "H": hole)
        steps_per_episode = []
        pi_track = []
        rewards = []
        ...
        for e in tqdm(range(n_episodes), leave=False):
            ...
            if random_start:
                if np.random.random() > 0.5:
                    ...
                    starting_state = np.random.randint(1, nS)
                
                state, info = self.env.reset(starting_state=starting_state)
            else:
            ...
            while not done:
                ...
                if done:
                    ...
                    if reward == 1 - (self.env.step_penalty if hasattr(self.env, "step_penalty") else 0):
                        endings.append("G")
                    else:
                        endings.append("H")
        ...
        return Q, V, pi, Q_track, pi_track, rewards, endings, steps_per_episode
        # return Q, V, pi, Q_track, pi_track, steps_per_episode
