    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(
        ...
        def update_probability_matrix(row, col, action):
            ...
            if self.custom_rewards is not None:
                reward = self.custom_rewards.get(newletter, reward)
            else:
    ...
    def reset(
        ...
        *,
        ...
        starting_state: Optional[int] = None,
    ):
        super().reset(seed=seed)
        if starting_state is not None:
            if not (0 <= starting_state < self.observation_space.n):
                raise ValueError(
                    f"Starting state {starting_state} is not in the observation space"
                )
            if self.desc.ravel()[starting_state] not in b"SF":
                starting_state = 0
